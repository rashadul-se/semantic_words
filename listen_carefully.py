...
class WordRecommenderApp:
    ...
    def render_sidebar(self) -> Tuple[str, int, bool, bool]:
        ...
        with st.sidebar:
            ...
            return query, top_n, submit_button, force_sklearn
    ...
    def run(self):
        ...
        query, top_n, submit_button, force_sklearn = self.render_sidebar()
        ...
        if not st.session_state.initialized:
            ...
            # Create processor
            processor, processor_type = ProcessorFactory.create_processor(prefer_pyspark=not force_sklearn)
        ...
        if submit_button and query.strip():
            try:
                with st.spinner("🔍 Searching for similar words..."):
                    results = processor.find_similar(query, top_n)
                
                if len(results) > 0:
                    self.display_results(results)
                else:
                    st.warning("⚠️ No similar words found. Try a different query.")
            ...
            # Show instructions
            st.info("👈 Enter a definition in the sidebar and click 'Find Similar Words' to get started!")
            
            with st.expander("ℹ️ How to use"):
                st.markdown("""
                1. **Select a query type** from the sidebar (or enter your own custom query)
                2. **Adjust the number of results** you want to see
                3. **Click 'Find Similar Words'** to get recommendations
                4. **Explore results** in different visualizations (Bar Chart, Line Chart, Data Table)
                5. **Download results** as CSV if needed
                
                **Example queries:**
                - "occurring or being done at the present moment"
                - "a person who teaches students"
                - "the act of moving quickly on foot"
                """)
